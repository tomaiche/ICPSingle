...
output "ibm_cloud_private_admin_url" {
  value = "<a href='https://${element(module.deployVM_singlenode.ipv4,count.index)}:8443' target='_blank'>https://${module.deployVM_singlenode.ipv4}:8443</a>"
}
...
output "ibm_cloud_private_master_ip" {
  value = "$(element(module.deployVM_singlenode.ipv4,count.index)}"
}
